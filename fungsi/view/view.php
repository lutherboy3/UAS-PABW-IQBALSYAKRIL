	 ...
	 */ 
	 class view {
		...
		function __construct($db){
			...
			function barang(){
				$sql = "select barang.*, supplier.id_supplier, supplier.supplier
			...
			function barang_stok(){
				$sql = "select barang.*, supplier.id_kategori, supplier.nama_kategori
						from barang inner join supplier on barang.id_kategori = supplier.id_kategori 
						where stok <= 3 
						ORDER BY id DESC";
				$row = $this-> db -> prepare($sql);
				$row -> execute();
			...
			function all_report($kode_supplier){
				$sql ="
				SELECT nota.* , LP_PenjualanRefan.barang.id_barang, barang.nama_barang,barang.disc, member.id_member,
				member.nm_member,supplier.supplier,barang.harga_jual,barang.harga_jual-((barang.disc/100)*barang.harga_jual) as subtotal  from LP_PenjualanRefan.nota 
			   left join LP_PenjualanRefan.barang on barang.id_barang=nota.id_barang 
			   left join LP_PenjualanRefan.member on member.id_member=nota.id_member
               left join LP_PenjualanRefan.supplier on supplier.id_supplier=barang.id_supplier
			   where supplier.supplier='$kode_supplier'
			   ORDER BY id_nota";
				$row = $this-> db -> prepare($sql);
				$row -> execute();
				$hasil = $row -> fetchAll();
